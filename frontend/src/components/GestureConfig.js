...
function GestureConfig({ gestureMappings, onMappingUpdate }) {
  ...
  const supportedGestures = [
    'wave', 'fist', 'open_palm', 'thumbs_up', 'peace', 'pointing',
    'rock_sign', 'ok_sign', 'call_sign', 'middle_finger', 'ring_finger',
    'pinky', 'three_fingers', 'three_fingers_serbian_style', 'two_fingers_ir',
    'two_fingers_mr', 'four_fingers'
  ];
  ...
  const handleDelete = async (gesture) => {
    if (!window.confirm(`Are you sure you want to delete the mapping for ${gesture}?`)) {
      return;
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      await ApiService.updateGestureMapping(formData.gesture, formData.action, formData.params);
  ...
  const unusedGestures = supportedGestures.filter(gesture => !gestureMappings[gesture]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Gesture Configuration</h1>
            <p className="text-gray-600 mt-1">Configure which actions are triggered by each gesture</p>
          </div>
          <button
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">Gesture</label>
                <select
                  className="input-field"
                  value={formData.gesture}
                  onChange={(e) => setFormData(prev => ({ ...prev, gesture: e.target.value }))}
                  required
                  disabled={editingGesture !== null}
                >
                  <option value="">Select a gesture</option>
                  {(editingGesture ? [editingGesture] : unusedGestures).map(gesture => (
            ...
            {formData.action && (
              <div className="space-y-4">
                <h4 className="text-sm font-medium text-gray-700">Action Parameters</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {/* Current Mappings */}
      <div className="card">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Current Mappings</h3>
        
        {Object.keys(gestureMappings).length === 0 ? (
          <div className="text-center py-8">
            <Settings className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <p className="text-gray-500">No gesture mappings configured</p>
            <p className="text-sm text-gray-400">Add your first mapping above to get started</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(gestureMappings).map(([gesture, mapping]) => (
              <div key={gesture} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      {mapping.params && Object.keys(mapping.params).length > 0 && (
                        <p className="text-xs text-gray-500 mt-1">
                          {Object.entries(mapping.params).map(([key, value]) => 
                            `${key}: ${value}`
                          ).join(', ')}
                        </p>
                      )}
      ...
      <div className="card">
        ...
        <p className="text-gray-600 mb-4">These gestures can be detected but don't have actions mapped yet:</p>
        
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {unusedGestures.map(gesture => (
            <div key={gesture} className="bg-gray-100 rounded-lg p-3 text-center">
